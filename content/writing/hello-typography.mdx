...
Code blocks render through `rehype-pretty-code` at build time, themed with Vesper. A short TypeScript example:
...
## Media routes through Bunny CDN

Images and videos are delivered from two Bunny pull zones — `images.francois.works` and `videos.francois.works` — through a custom `next/image` loader that requests WebP at the density the viewport actually needs. Authoring stays MDX-shaped.

<Figure
  src="/test.jpg"
  alt="A 1200-square smoke-test image served from Bunny"
  width={1200}
  height={1200}
>
  <Caption>
    Served as WebP via Bunny's Image Processing query params. No client-side
    optimization runs.
  </Caption>
</Figure>

Plain markdown images flow through the same loader:

![A smaller inline variant of the same image](/test.jpg)

Short autoplay loops fit `<Video>`. The defaults — `autoplay`, `muted`, `loop`, `playsInline` — cover the common case; override per prop when an article needs controls or a manual start.

<Video src="/test.mp4" width={1280} height={720}>
  <Caption>A short autoplay loop served from Bunny.</Caption>
</Video>

GIFs work but are discouraged: an MP4 of the same clip is typically 10–100× smaller. When a `.gif` does appear, the loader passes it through untouched so animation survives.
